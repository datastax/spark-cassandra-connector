...
class WriteOptionTest extends FlatSpec with Matchers {
  ...
  it should "properly create auto duration" in {
    val option = TTLOption.auto
    option should be(AutoWriteOption)
  }
  ...
  it should "properly create per-row duration placeholder" in {
    ...
    option.placeholder should be("test")
  }
  ...
  it should "properly create constant write option with DateTime" in {
    ...
    option shouldBe a[StaticWriteOption[_]]
    option.value should be(new DateTime(2010, 5, 6, 7, 8, 8, 10).getMillis * 1000L)
  }
  ...
  it should "properly create constant write option with Date" in {
    ...
    option shouldBe a[StaticWriteOption[_]]
    option.value should be(t.getTime * 1000L)
  }

  it should "properly create auto timestamp" in {
    val option = TimestampOption.auto
    option should be(AutoWriteOption)
  }
