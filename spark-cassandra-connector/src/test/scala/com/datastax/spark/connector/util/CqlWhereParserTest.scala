...
class CqlWhereParserTest extends FlatSpec with Matchers {
  ...
  "CqlWhereParser" should "parse 'and' operations" in {
    parser.predicates("abc > 10 and cde > 20 and c < 1", Seq())(1).columnName should be("cde")
  }

  it should "parse in operations" in {
    parser.predicates("cde in  (20, 30)", Seq())(0).columnName should be("cde")
  }
  ...
  it should " parse strings" in {
    parser.predicates(" Cde in  ('20', '30')", Seq())(0) match {
  ...
  it should " return UnknownPredicate on error" in {
    parser.predicates(" Cde inasa  ('20', '30')", Seq())(0) match {
      case  UnknownPredicate(_, data, values) => data should be ("Cde inasa  ('20', '30')")
    }
  }

  it should " return UnknownPredicate on error in second expr" in {
    parser.predicates("a = 10 and  Cde inasa  ('20', ?)", Seq(1))(1) match {
      case  UnknownPredicate(_, data,values) => {
        data should be("Cde inasa  ('20', ?)")
        values.length should be (1)
      }
    }
  }
  it should " accept >= " in {
    parser.predicates("abc >= ?", Seq(1)).length should be(1)
  }

  it should " accept ?" in {
    parser.predicates("abc in  ?", Seq((1)))(0) match {
      case InPredicate(name, value) => name should be("abc"); value should be (1)
      case _ => assert(false)
    }
  }

  it should " accept name with quotes and other special symbols" in {
    parser.predicates("\"\"\"ab!@#c\" in ?",Seq((1,2)))(0).columnName should be("\"\"ab!@#c")
  }

  it should " accept param with quotes and other special symbols" in {
    parser.predicates("abc > 'abc''abc$%^'", Seq())(0) match {
  ...
  it should " accept uuid param" in {
    parser.predicates("abc > 01234567-0123-0123-0123-0123456789ab", Seq())(0) match {
  ...
  it should " accept float param" in {
    parser.predicates("abc >-12.e+10", Seq())(0) match {
  ...
  it should "parse case insensitive 'IN' operations ?" in {
    parser.predicates("cde IN  ?", Seq((1)))(0).columnName should be("cde")
  }
}
