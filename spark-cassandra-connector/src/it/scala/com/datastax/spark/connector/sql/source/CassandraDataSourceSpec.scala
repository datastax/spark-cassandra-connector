...
class CassandraDataSourceSpec extends SparkCassandraITFlatSpecBase {
  ...
  conn.withSessionDo { session =>
    ...
    session.execute("CREATE KEYSPACE IF NOT EXISTS sql_test2 WITH REPLICATION = " +
      "{ 'class': 'SimpleStrategy', 'replication_factor': 1 }")
  ...
  it should "allow to insert null" in {
    ...
    createTempTable("sql_test", "null_test", "null_test")
    createTempTable("sql_test", "null_test2", "null_test2")
    sqlContext.sql("select * from null_test where d = 'one' ").collect() should have length 1
    sqlContext.sql("select * from null_test where d IS NULL ").collect() should have length 1
    sqlContext.sql("select * from null_test where d IS NOT NULL ").collect() should have length 2
    sqlContext.sql("select * from null_test where v > 1 ").collect() should have length 2
    sqlContext.sql("INSERT OVERWRITE TABLE null_test2 SELECT k, v, d FROM null_test")
    sqlContext.sql("select * from null_test2").collect() should have length 3
    sqlContext.dropTempTable("null_test2")
    sqlContext.dropTempTable("null_test")
  }
  ...
  it should "allow to query on table with upper case names" in {
    ...
    createTempTable("sql_test", "Upper_Case_Table", "Upper_case_test")
    sqlContext.sql("select KEY, VALUE from Upper_case_test where VALUE > 1").collect() should have length 2
    sqlContext.dropTempTable("Upper_case_test")
  }
}
