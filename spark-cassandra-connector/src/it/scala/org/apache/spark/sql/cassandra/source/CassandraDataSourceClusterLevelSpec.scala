...
class CassandraDataSourceClusterLevelSpec  extends SparkCassandraITFlatSpecBase {
  ...
  conn.withSessionDo { session =>
    session.execute("CREATE KEYSPACE IF NOT EXISTS sql_ds_test1 WITH REPLICATION = { 'class': 'SimpleStrategy', 'replication_factor': 1 }")

    session.execute("CREATE TABLE IF NOT EXISTS sql_ds_test1.test1 (a INT PRIMARY KEY, b INT, c INT)")
    session.execute("USE sql_ds_test1")
    session.execute("INSERT INTO sql_ds_test1.test1 (a, b, c) VALUES (1, 1, 1)")
    session.execute("INSERT INTO sql_ds_test1.test1 (a, b, c) VALUES (2, 1, 2)")
    session.execute("INSERT INTO sql_ds_test1.test1 (a, b, c) VALUES (3, 1, 3)")
    session.execute("INSERT INTO sql_ds_test1.test1 (a, b, c) VALUES (4, 1, 4)")
    session.execute("INSERT INTO sql_ds_test1.test1 (a, b, c) VALUES (5, 1, 5)")
  }

  val conn2 = CassandraConnector(Set(getHost(1)), getNativePort(1))
  conn2.withSessionDo { session =>
    session.execute("CREATE KEYSPACE IF NOT EXISTS sql_ds_test2 WITH REPLICATION = { 'class': 'SimpleStrategy', 'replication_factor': 1 }")

    session.execute("CREATE TABLE IF NOT EXISTS sql_ds_test2.test2 (a INT PRIMARY KEY, d INT, e INT)")
    session.execute("USE sql_ds_test2")
    session.execute("INSERT INTO sql_ds_test2.test2 (a, d, e) VALUES (8, 1, 8)")
    session.execute("INSERT INTO sql_ds_test2.test2 (a, d, e) VALUES (7, 1, 7)")
    session.execute("INSERT INTO sql_ds_test2.test2 (a, d, e) VALUES (6, 1, 6)")
    session.execute("INSERT INTO sql_ds_test2.test2 (a, d, e) VALUES (4, 1, 4)")
    session.execute("INSERT INTO sql_ds_test2.test2 (a, d, e) VALUES (5, 1, 5)")
    session.execute("CREATE TABLE IF NOT EXISTS sql_ds_test2.test3 (a INT PRIMARY KEY, d INT, e INT)")
  }
  ...
  override def beforeAll() {
    ...
    createTempTable("sql_ds_test1", "test1", "cluster1", "test1")
    createTempTable("sql_ds_test2", "test2", "cluster2", "test2")
    createTempTable("sql_ds_test2", "test3", "cluster2", "test3")
  }

  override def afterAll() {
    super.afterAll()
    conn.withSessionDo { session =>
      session.execute("DROP KEYSPACE sql_ds_test1")
    }
    conn2.withSessionDo { session =>
      session.execute("DROP KEYSPACE sql_ds_test2")
    }
