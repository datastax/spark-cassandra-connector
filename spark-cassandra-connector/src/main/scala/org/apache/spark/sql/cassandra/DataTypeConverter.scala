...
object DataTypeConverter extends Logging {
  ...
  def catalystDataType(cassandraType: connector.types.ColumnType[_], nullable: Boolean): catalystTypes.DataType = {
    ...
    cassandraType match {
      case connector.types.SetType(et)                => catalystTypes.ArrayType(catalystDataType(et, nullable), false)
      case connector.types.ListType(et)               => catalystTypes.ArrayType(catalystDataType(et, nullable), false)
      case connector.types.MapType(kt, vt)            => catalystTypes.MapType(catalystDataType(kt, nullable), catalystDataType(vt, nullable), false)
