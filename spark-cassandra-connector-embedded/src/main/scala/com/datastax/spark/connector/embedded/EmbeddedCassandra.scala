    ...
    if (getProperty(HostProperty).isEmpty) {
      ...
      for (i <- configTemplates.indices) {
        ...
        if (templatePairs(i)._2 != templatePairs(i)._1 || forceReload) {
          cassandraRunners.lift(i).foreach(_.foreach(_.destroy()))
...
object UserDefinedProperty {
  ...
  trait IntProperty extends TypedProperty {
    ...
    def checkValueType(obj: Any) =
      obj match {
        case x: Int => x
        case _ => throw new ClassCastException (s"Expected Int, got ${obj.getClass.getName}")
      }
  }
  ...
  trait InetAddressProperty extends TypedProperty {
    ...
    def checkValueType(obj: Any) =
      obj match {
        ...
        case _ => throw new ClassCastException (s"Expected InetAddress, got ${obj.getClass.getName}")
      }
  }
...
object EmbeddedCassandra {
  ...
  Runtime.getRuntime.addShutdownHook(new Thread(new Runnable {
    override def run() = cassandraRunners.foreach(_.foreach(_.destroy()))
  }))
}
