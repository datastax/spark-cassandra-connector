...
object DseCassandraConnectionFactory extends CassandraConnectionFactory {
  ...
  def dseClusterBuilder(conf: CassandraConnectorConf) = {
    ...
    val dseBuilder = DseCluster.builder()
      ...
      .withoutGeospatialCodecs() // We will handle this in the singleton DseTypeConverter
  ...
  private def getMetaData(columnNames: IndexedSeq[String], result: ContinuousPagingResult) = {
    ...
    val codecs = columnDefs
      .map(col => CodecRegistry.DEFAULT_INSTANCE.codecFor(col.getType))
