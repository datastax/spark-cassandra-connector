...
public class SchemaManagerService
{
    ...
    /*
     * there are tree states of dse_graph DB.
    ...
    public void refreshMetadata()
    {
        ...
        try
        {
            ...
            try
            {
                ...
                graphNames = Optional.of(StreamSupport.stream(rs.spliterator(), false).map(r -> r.asString())
    ...
    public List<String> findUnmappedKeyspaces(CassandraHiveMetaStore cassandraHiveMetaStore) {
        ...
        for (KeyspaceMetadata ksMetadata : getClusterMetadata().getKeyspaces())
        {
            ...
            if (isInternalKeyspace(ksName) || isGraphKs(ksName) || isKeyspaceMapped(ksName, cassandraHiveMetaStore))
                continue;
    ...
    private boolean isGraphKs(String ksName)
    {
        return ksName.endsWith("_pvt") || ksName.endsWith("_system") || graphNames.orElse(Collections.EMPTY_SET).contains(ksName);
    }
