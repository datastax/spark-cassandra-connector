...
class CassandraJavaUtilSpec extends FlatSpec with Matchers with BeforeAndAfter with CassandraServer with SparkServer {
  ...
  it should "allow to save nested beans to Cassandra" in {
    ...
    val beansRdd = sc.parallelize(Seq(
      new outer.SampleNestedJavaBean(1, "one"),
      new outer.SampleNestedJavaBean(2, "two"),
      new outer.SampleNestedJavaBean(3, "three")
    ))

    CassandraJavaUtil.javaFunctions(beansRdd, classOf[outer.SampleNestedJavaBean])
      .saveToCassandra("java_api_test", "test_table", CassandraJavaUtil.NO_OVERRIDE)
  ...
  it should "throw IllegalArgumentException when trying to read data as member Java beans" in {
    conn.withSessionDo { session =>
      session.execute("INSERT INTO java_api_test.test_table (key, value) VALUES (1, 'one')")
      session.execute("INSERT INTO java_api_test.test_table (key, value) VALUES (2, 'two')")
      session.execute("INSERT INTO java_api_test.test_table (key, value) VALUES (3, 'three')")
    ...
    intercept[IllegalArgumentException](CassandraJavaUtil.javaFunctions(sc)
      .cassandraTable("java_api_test", "test_table", classOf[SampleWithNestedJavaBean#SampleNestedJavaBean]).toArray())
  }
