...
class TwitterStreamingHashTagsByInterval extends Serializable {
  ...
  def start(auth: Option[Authorization], ssc: StreamingContext, tags: Set[String], keyspace: String, table: String): Unit = {
    ...
    val tagCountsByMinute = hashTags.map{case (hashtag, mentions) => (hashtag, mentions, "M" +now("yyyyMMddHHmm"))}
    val tagCountsByHour = hashTags.map{case (hashtag, mentions) => (hashtag, mentions, "H" + now("yyyyMMddHH"))}
    val tagCountsByDay  = hashTags.map{case (hashtag, mentions) => (hashtag, mentions, "D" + now("yyyyMMdd"))}
    val tagCountsAll    = hashTags.map{case (hashtag, mentions) => (hashtag, mentions, "ALL")}

    ssc.union(Seq(tagCountsByMinute, tagCountsByHour, tagCountsByDay, tagCountsAll))
