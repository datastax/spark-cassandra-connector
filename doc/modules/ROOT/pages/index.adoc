...
|https://mvnrepository.com/artifact/com.datastax.spark/spark-cassandra-connector_2.12/3.0.0-beta[3.0.0-beta]
|===

== Features

_Lightning-fast cluster computing with Apache Spark(TM) and Apache
Cassandra(R)._
...
below)
* Compatible with Apache Spark 1.0 through 3.0 (see table below)
...
There are two test suites in the Connector: unit tests and integration tests.
...
To run the integration tests, first install https://github.com/riptano/ccm[CCM]:

....
pip install ccm
....

Make sure you don't have the `DSE_HOME` environment variable set. If you are on OS X, you must run the following script to create the local interface aliases used by CCM before running the integration tests:

....
./testing/scripts/mac-localhost-aliases.sh
....
