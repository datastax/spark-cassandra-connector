...
when running stages, the numbers X/X in this progress bar represent tasks completed/ tasks total.
...
Spark partitions(tasks) to be created is directly controlled by the setting `spark.cassandra.input.split.size`.
...
If there are more than one tasks but only a single machine is working make sure that the job itself
...
to your Spark Cassandra Connector assembly jar. If this is impossible for some reason the second best option
is to manually distribute the jar to all of your executors and add the jar's location to the executor classPath
...
For those you should set the options in the `SparkConf` object used to 
...
This is usually means that the size of the partitions you are attempting to create are larger than
...
If you are not using Spark Submit (not recommended) make sure that your dependencies have been set
in the `SparkConf` using `setJars` or by distributing the jars yourself and modifying the executor
 classpath.
...
When troublshooting it is sometimes useful to set this ti `0.0.0.0` so any incoming connection will
work.
...
would like to add. We would be happy to discuss it with you and see your work. When you have a branch
