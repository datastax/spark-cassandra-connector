...
class DateRangeTypeSpec extends SparkCassandraITFlatSpecBase with DefaultCluster {
  ...
  "The Spark Cassandra Connector" should "find a converter for DateRange types" in {
    ColumnType.fromDriverType(DseDataTypes.DATE_RANGE) should be(DateRangeType)
  }

  it should "read DateRange types" in {
  ...
  it should "write DateRange types" in {
  ...
  "SparkSql" should "read DateRange types" in {
